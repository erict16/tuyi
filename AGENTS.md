...
Preferred CLI is `python -m tuyi` (frozen names `tuyi` / `tuyi-cli`). User data lives under `~/.tuyi_*`. Older Honsen filenames listed as `LEGACY_*` in `backend/app_meta.py` migrate once; do not write new files under those names.
