...
class DrawingsApiTests(unittest.TestCase):
    ...
    def test_frontend_batch_and_glossary_controls(self):
        ...
        self.assertIn("这张图用术语表", text)
        self.assertIn("导出术语", text)
        self.assertIn("我定的译法", text)
        ...
        self.assertIn("打不开 DWG 时改写成 DXF", text)
        self.assertIn("块里面的字", text)
        ...
        self.assertIn("不是原文那种语言", text)
        self.assertIn("图纸上只留译文", text)
        self.assertIn("原文和译文都留", text)
        self.assertIn("全部文字", text)
        self.assertIn("去掉重复", text)
